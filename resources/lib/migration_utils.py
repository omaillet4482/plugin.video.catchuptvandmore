...
def update_json(key, order, s, j, setting, item_infos):
    set_setting = True
    if setting == "":
        set_setting = False
    else:
        if order:
            if item_infos['order'] == int(setting):
                set_setting = False
        else:
            if bool(setting):
                set_setting = False
    ...
    if set_setting:
        ...
        if s not in j[key]:
            j[key][s] = {}
        if order:
            j[key][s]['order'] = int(setting)
        else:
            j[key][s]['hidden'] = not bool(setting)
        ...
        if 'id' in elem.attrib:
            ...
            for skeleton in skeletons:
                ...
                if setting_id in current_menu:
                    ...
                    update_json(skeleton, order, setting_id, j, setting_value, item_infos)
