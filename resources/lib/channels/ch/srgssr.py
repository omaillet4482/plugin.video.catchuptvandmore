...
# search items
URL_LIST_SEARCH = 'https://il.srgssr.ch/integrationlayer/2.0/%s/searchResultShowList?q=%s&mediaType=video&pageSize=20&vector=portalplay'
...
def list_search(plugin, search_query, item_id, page, **kwargs):
    resp = urlquick.get(URL_LIST_SEARCH % (item_id[:3], search_query), headers=GENERIC_HEADERS, max_age=-1).content.decode()
    json_parser = json.loads(resp)
...
def get_video_url(plugin, item_id, video_id, download_mode=False, **kwargs):
    ...
    resp = urlquick.get(URL_INFO_VIDEO % (channel_name_value, video_id), max_age=-1).content.decode()
    json_parser = json.loads(resp)
...
def get_final_video_url(plugin, video_assets, video_id):
    ...
    response = json.loads(urlquick.get(URL_TOKEN, params={"acl": acl}, max_age=-1).content.decode())
    if "?" not in manifest:
        manifest += "?"
    else:
        manifest += "&"
...
def get_live_url(plugin, item_id, **kwargs):
    resp = urlquick.get(URL_LIVE_JSON % (item_id[:3], item_id[:3]), max_age=-1)
    ...
    resp2 = urlquick.get(URL_INFO_VIDEO % (item_id[:3], live_id), max_age=-1)
    json_parser2 = json.loads(resp2.text)

    subtitle_url = None
    protocol, final_video_url, license_url = get_final_video_url(plugin, json_parser2, live_id)
